...
def drow_box_with_4points(image, label, fontScale = 0.7):
    ...
    for i in range(5,13,2):
        ...
        cv2.circle(image, (int(x), int(y)), 4, (0, 255, 0), thickness=-1)
...
def drawing(image, image_path, label_path, label_loaded):
    ...
    if g_print_help:
        text = "[h]:print this help,[Esc]:exit,[0~9]:class,[ ]:save,[w]:next,[q]:previous,[r]:undo,[Del/d]:del"
        words = text.split(',')
        for i in range(len(words)):
            cv2.putText(image_show, words[i], (0, (i + 1) * line_h), cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale, (0, 255, 255), thickness)
    else:
        cv2.putText(image_show, '[-h] for help', (0, line_h), cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale, (0, 255, 255), thickness)
    p = image_show.shape[0] - 3
    for i in range(len(className)):
        if i == currentClassID:
            cv2.putText(image_show, '{}:{}'.format(i, className[i]), (0, p - i * line_h), cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale, (0, 255, 0), thickness)
        else:
            cv2.putText(image_show, '{}:{}'.format(i, className[i]), (0, p - i * line_h), cv2.FONT_HERSHEY_COMPLEX_SMALL, fontScale, (0, 255, 255), thickness)
    
    if isfile(label_path) and not label_loaded[0]:
...
def on_EVENT_LBUTTONDOWN(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if 8 == len(tmpPoints):
            ...
            tmpPoints = []
        drawing(param[0], param[1], param[2], param[3])
...
def main(args):
    ...
    if args.skip:
        ...
        image_files = [f for f in listdir(args.original_image_dir) if f not in labeled_images and isfile(join(args.original_image_dir, f))]
    else:
    ...
    while len(image_files) > 0 and imgIdx >= 0 and imgIdx < len(image_files):
        ...
        if image is None:
            ...
            send2trash(original_image_path)
            image_files.remove(image_name)
            continue
        
        resized_image = soft_resize(image, g_image_size)
        label_loaded = [False]
        cv2.setMouseCallback("image", on_EVENT_LBUTTONDOWN, param=[resized_image, labeled_image_path, label_path, label_loaded])
        drawing(resized_image, labeled_image_path, label_path, label_loaded)
        while True:
            key = cv2.waitKey()
            if -1 != key:
                print(key)
            ...
            if 48 <= key and len(className) + 48 > key: # 0~9
                ...
                # if len(tmpPoints) == 0 and tmpLabels.size:
                #     tmpLabels[-1, 0] = currentClassID
                drawing(resized_image, labeled_image_path, label_path, label_loaded)

            if 32 == key: # /space
                cv2.imwrite(labeled_image_path, resized_image)
                if not tmpLabels.size:
                    continue
            ...
            if 255 == key or 100 == key: # del image
                ...
                if isfile(labeled_image_path):
                    ...
                    send2trash(labeled_image_path)
                if isfile(label_path):
                    # os.remove(label_path)
                    send2trash(label_path)
                image_files.remove(image_name)
